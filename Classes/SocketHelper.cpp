...
void SocketHelper::connect()
{
    ...
    //连接服务器
    res=this->s_client.Connect("192.168.1.133", 9000);
...
//        std::cout << "recv --> "<<buf<<std::endl;
        std::string str = buf;
        cocos2d::Director::getInstance()->getScheduler()->performFunctionInCocosThread([this,buf]
                                                {
                                                    cocos2d::EventCustom event("SocketMessage");
                                                    event.setUserData((void *) buf);
                                                    cocos2d::Director::getInstance()->getEventDispatcher()->dispatchEvent(&event);
//                                                    KinectHelper::getInstance()->analyzeAndDispatchKinectData(std::string(buf));
                                                });
    }
    //断开链接了
    this->s_client.Close();
